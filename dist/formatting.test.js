...
describe('formatOutbound', () => {
    const waChannel = { prefixAssistantName: true };
    const noPrefixChannel = { prefixAssistantName: false };
    const defaultChannel = {};
    it('prefixes with assistant name when channel wants it', () => {
        expect(formatOutbound(waChannel, 'hello world')).toBe(`${ASSISTANT_NAME}: hello world`);
    });
    ...
    it('defaults to prefixing when prefixAssistantName is undefined', () => {
        expect(formatOutbound(defaultChannel, 'hello world')).toBe(`${ASSISTANT_NAME}: hello world`);
    });
    ...
    it('strips internal tags and prefixes remaining text', () => {
        expect(formatOutbound(waChannel, '<internal>thinking</internal>The answer is 42')).toBe(`${ASSISTANT_NAME}: The answer is 42`);
    });
});
